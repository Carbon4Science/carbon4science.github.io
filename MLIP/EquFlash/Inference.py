"""EquFlash (SevenNet + FlashTP) inference module for MLIP benchmarking."""
...
def _get_calculator(device=None, checkpoint_path=None):
    ...
    if checkpoint_path:
        calc = SevenNetCalculator(
            model=checkpoint_path, modal="mpa", device=device, enable_flash=True,
        )
    else:
        calc = SevenNetCalculator(
...
def run(input_data, top_k: int = 10) -> List[Dict]:
    """
    Run NVT MD simulation using EquFlash (SevenNet + FlashTP) calculator.
