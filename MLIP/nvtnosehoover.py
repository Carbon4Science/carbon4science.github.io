...
from MLIP.nose import get_nose_q
...
class NVTNoseHoover(MolecularDynamics):
    ...
    def __init__(
        ...
        if nose_frequency is None:
            nose_frequency = 40 * timestep / units.fs
        self.nose_q = get_nose_q(atoms=atoms, unit="fs", temperature=self.temp, frequency=nose_frequency)
        #print("Nose Q:", self.nose_q/(units.fs**2), "eV fs2")
        self.dt = timestep
    ...
    def step(self):
        ...
        nvt_bath_halfstep = self.nvt_bath + 0.5 * self.dt * e_kin_diff / self.nose_q
        e_kin_diff_halfstep = 0.5 * (
            np.sum(masses * np.sum(vel_halfstep**2, axis=1))
            - (3 * self.natoms + 1) * units.kB * self.temp
        )
        self.nvt_bath = (
            nvt_bath_halfstep + 0.5 * self.dt * e_kin_diff_halfstep / self.nose_q
        )
