"""Allegro-OAM-L inference module for MLIP benchmarking."""
...
def _get_calculator(device=None):
    ...
    compiled_path = os.path.join(os.path.dirname(__file__), "Allegro-OAM-L.nequip.pt2")
...
def run_production(config_path, structure_index=0, carbon_tracker=None):
    ...
    md_info = run_md_simulation(struct_cfg, calculator=calc, model_name="Allegro", carbon_tracker=carbon_tracker)
    timing = {"equil_seconds": md_info["equil_seconds"], "prod_seconds": md_info["prod_seconds"]}
    analysis = run_analysis("Allegro", struct_cfg, timing=timing)
    return {**md_info, "accuracy": analysis.get("accuracy", {})}


def run(input_data, top_k: int = 10) -> List[Dict]:
    """
    Run NVT MD simulation using Allegro-OAM-L calculator.
