"""Nequix MP inference module for MLIP benchmarking."""
...
def _get_calculator(device=None):
    ...
    # JAX uses CUDA_VISIBLE_DEVICES for device selection
    if device is not None and device.startswith("cuda:"):
        gpu_id = device.split(":")[1]
        os.environ["CUDA_VISIBLE_DEVICES"] = gpu_id
    elif device is None:
        # Default: use first GPU if available
        pass
...
def run_production(config_path, structure_index=0, carbon_tracker=None):
    ...
    md_info = run_md_simulation(struct_cfg, calculator=calc, model_name="Nequix", carbon_tracker=carbon_tracker)
    timing = {"equil_seconds": md_info["equil_seconds"], "prod_seconds": md_info["prod_seconds"]}
    analysis = run_analysis("Nequix", struct_cfg, timing=timing)
    return {**md_info, "accuracy": analysis.get("accuracy", {})}


def run(input_data, top_k: int = 10) -> List[Dict]:
    """
    Run NVT MD simulation using Nequix calculator.
