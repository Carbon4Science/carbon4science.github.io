"""PET-OAM-XL inference module for MLIP benchmarking."""
...
def _get_calculator(device=None, checkpoint_path=None):
    ...
    if checkpoint_path:
        ...
        calc = MetatomicCalculator(checkpoint_path, device=device)
    else:
        from upet.calculator import UPETCalculator
        calc = UPETCalculator(model="pet-oam-xl", version="1.0.0", device=device)
...
def run_production(config_path, structure_index=0, track_carbon=False, checkpoint_path=None):
    ...
    md_info = run_md_simulation(struct_cfg, calculator=calc, model_name="PET", track_carbon=track_carbon)
    timing = {"equil_seconds": md_info["equil_seconds"], "prod_seconds": md_info["prod_seconds"]}
    analysis = run_analysis("PET", struct_cfg, timing=timing)
    return {**md_info, "accuracy": analysis.get("accuracy", {})}


def run(input_data, top_k: int = 10) -> List[Dict]:
    """
    Run NVT MD simulation using PET-OAM-XL calculator.
