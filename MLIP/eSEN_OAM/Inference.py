"""eSEN-30M-OAM inference module for MLIP benchmarking."""
...
def _get_calculator(device=None):
    ...
    checkpoint_path = hf_hub_download(
        repo_id="facebook/OMAT24", filename="esen_30m_oam.pt"
    )
...
def run_production(config_path, structure_index=0, carbon_tracker=None):
    ...
    md_info = run_md_simulation(struct_cfg, calculator=calc, model_name="eSEN_OAM", carbon_tracker=carbon_tracker)
    timing = {"equil_seconds": md_info["equil_seconds"], "prod_seconds": md_info["prod_seconds"]}
    analysis = run_analysis("eSEN_OAM", struct_cfg, timing=timing)
    return {**md_info, "accuracy": analysis.get("accuracy", {})}


def run(input_data, top_k: int = 10) -> List[Dict]:
    """
    Run NVT MD simulation using eSEN-30M-OAM calculator.
