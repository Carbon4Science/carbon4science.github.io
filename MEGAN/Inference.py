"""
MEGAN retrosynthesis prediction inference.
...
_DEFAULT_CONFIG = {
    'save_path': os.path.join(_REPO_DIR, 'models', 'uspto_50k'),
    'beam_size': 50,
    'max_gen_steps': 16,
    'n_max_atoms': 200,
    'device': 'cuda:0',
...
def run(smiles: Union[str, List[str]], top_k: int = 10) -> List[Dict]:
    """
    Predict reactants from product using MEGAN.
    ...
    for smi in smiles_list:
        try:
            ...
            formatted_preds = []
            seen = set()
            for path in beam_results[0]:
                raw_smi = path.get('final_smi_unmapped', '')
                if not raw_smi:
                    continue

                # Canonicalize
                pred_mol = Chem.MolFromSmiles(raw_smi)
                if pred_mol is None:
                    continue
