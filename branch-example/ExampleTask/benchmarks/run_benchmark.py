...
When setting up a new task, replace "ExampleTask" with your task name
throughout this file.

Usage:
    python run_benchmark.py --model ExampleModel --track_carbon
    python run_benchmark.py --model ExampleModel --limit 500
"""
...
# ── Task configuration ─────────────────────────────────────────────────────
# Replace "ExampleTask" with your task name.
# Add one entry per model under "models".
TASK_NAME = "ExampleTask"

EVAL_MODULE = "evaluate"  # points to <Task>/evaluate.py

MODELS = {
    # "ModelName": "ModelName.Inference",
    "ExampleModel": "ExampleModel.Inference",
}

MODEL_ENVS = {
    # "ModelName": "conda_env_name",
...
def count_model_parameters(model_name: str) -> Optional[int]:
    """Count trainable parameters after model is loaded."""
    try:
        import torch
...
def run_benchmark(
    ...
    for i, tc in enumerate(test_cases):
        try:
            result = model_run(tc['input'], top_k=top_k)
            predictions.append(result[0] if result else {'input': tc['input'], 'predictions': []})
        except Exception:
            predictions.append({'input': tc['input'], 'predictions': []})
